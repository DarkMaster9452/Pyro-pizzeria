            ...
            <AnimatePresence initial={false}>
              {list.map((o) => (
                ...
                >
                  ...
                  {/* When the phone field isn't a real number, show whatever was
                      written (often a name) instead of a call button, so nobody
                      dials a name by mistake. */}
                  {!o.callable && (
                    <p className="mt-2 flex items-center gap-1.5 rounded-xl border border-amber-400/40 bg-amber-400/10 px-3 py-2 text-xs font-semibold text-amber-300">
                      <Phone className="h-4 w-4 shrink-0" />
                      {o.phone
                        ? `V telefóne bolo napísané: ${o.phone}`
